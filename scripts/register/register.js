const form = document.getElementById("registerForm");

const alertBox = document.getElementById("alertBox");
const alertText = document.getElementById("alertText");

const users = [
  {
    email: "admin@gmail.com",
    password: "admin",
  },
  {
    email: "user1234@gmail.com",
    password: "1234",
  },
];

form.addEventListener("submit", function (e) {
  e.preventDefault();

  const email = document.getElementById("email").value;
  const password = document.getElementById("password").value;
  const confirmPassword = document.getElementById("confirmPassword").value;

  // reset alert
  alertBox.classList.remove("hidden");

  if (password !== confirmPassword) {
    showError("Wachtwoorden komen niet overeen!");
    return;
  }

  const exists = users.find((u) => u.email === email);
  if (exists) {
    showError("Deze gebruiker bestaat al!");
    return;
  }

  showSuccess("Account succesvol aangemaakt!");

  setTimeout(() => {
    window.location.href = "./home.html";
  }, 1500);
});

function showError(message) {
  alertBox.classList.remove(
    "bg-green-900",
    "border-green-700",
    "text-green-300",
  );
  alertBox.classList.add("bg-red-900", "border-red-700", "text-red-300");

  alertText.innerHTML = `<span class='font-medium'>Error:</span> ${message}`;
  hideAlertAfterDelay();
}

function showSuccess(message) {
  alertBox.classList.remove("bg-red-900", "border-red-700", "text-red-300");
  alertBox.classList.add("bg-green-900", "border-green-700", "text-green-300");

  alertText.innerHTML = `<span class='font-medium'>Success:</span> ${message}`;
  hideAlertAfterDelay();
}

function hideAlertAfterDelay() {
  setTimeout(() => {
    alertBox.classList.add("hidden");
  }, 3000); // 3 seconden
}
